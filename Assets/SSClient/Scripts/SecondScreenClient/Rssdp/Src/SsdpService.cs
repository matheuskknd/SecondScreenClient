...
namespace Rssdp {
	...
	public class SsdpService {
		...
		public SsdpService() {
			...
			ServiceTypeNamespace = SsdpConstants.UpnpDeviceTypeNamespace;
			ServiceVersion = 1;
		}
		...
		public SsdpService( string serviceDescriptionXml ) : this() {
			if( serviceDescriptionXml == null ) {
				throw new ArgumentNullException(nameof(serviceDescriptionXml));
			}

			if( serviceDescriptionXml.Length == 0 ) {
				throw new ArgumentException(nameof(serviceDescriptionXml) + " cannot be an empty string.",nameof(serviceDescriptionXml));
			}

			using( System.IO.MemoryStream ms = new System.IO.MemoryStream(System.Text.UTF8Encoding.UTF8.GetBytes(serviceDescriptionXml)) ) {
		...
		public string FullServiceType =>
				...
				string.Format("urn:{0}:service:{1}:{2}",
				(ServiceTypeNamespace ?? string.Empty).Replace(".","-"),
		...
		public string ServiceId =>
				...
				(
					"urn:{0}:serviceId:{1}",
					(ServiceTypeNamespace == SsdpConstants.UpnpDeviceTypeNamespace ? "upnp-org" : ServiceTypeNamespace).Replace(".","-"),
					Uuid ?? string.Empty
				);
		...
		private bool SetPropertyFromReader( XmlReader reader,SsdpService service ) {
			switch( reader.LocalName ) {
				...
				case "SCPDURL":
					ScpdUrl = StringToUri(reader.ReadElementContentAsString());
					break;

				case "controlURL":
					ControlUrl = StringToUri(reader.ReadElementContentAsString());
					break;

				case "eventSubURL":
					EventSubUrl = StringToUri(reader.ReadElementContentAsString());
					break;
		...
		private static void SetServiceTypePropertiesFromFullDeviceType( SsdpService service,string value ) {
			if( string.IsNullOrEmpty(value) || !value.Contains(":") ) {
				...
				string[] parts = value.Split(':');
				if( parts.Length == 5 ) {
					if( int.TryParse(parts[4],out int serviceVersion) ) {
