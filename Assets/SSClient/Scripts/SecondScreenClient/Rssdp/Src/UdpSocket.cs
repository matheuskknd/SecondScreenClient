...
namespace Rssdp {
	...
	internal sealed class UdpSocket : DisposableManagedObjectBase, IUdpSocket {
		...
		public System.Threading.Tasks.Task<ReceivedUdpData> ReceiveAsync() {
			...
			_Socket.ReceiveFromAsync(new System.ArraySegment<Byte>(state.Buffer), System.Net.Sockets.SocketFlags.None, state.EndPoint).ContinueWith((task, asyncState) => {
					...
					try{
						...
						if( !this.IsDisposed )
							throw;	//Only rethrow disposed exceptions if we're NOT disposed, because then they are unexpected.
					}
