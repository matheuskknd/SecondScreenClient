...
namespace Rssdp {
	...
	public sealed class DiscoveredSsdpDevice {
		...
		public bool IsExpired() =>
			CacheLifetime == TimeSpan.Zero || AsAt.Add(CacheLifetime) <= DateTimeOffset.Now;
		...
		public async Task<SsdpDevice> GetDeviceInfo() {
			...
			SsdpRootDevice device = _Device;

			if( device == null || IsExpired() )
		...
		public async Task<SsdpRootDevice> GetDeviceInfo( HttpClient downloadHttpClient ) {
			...
			if( _Device == null || IsExpired() ) {

				HttpResponseMessage rawDescriptionDocument = await downloadHttpClient.GetAsync(DescriptionLocation);
				...
				byte[] data = await rawDescriptionDocument.Content.ReadAsByteArrayAsync();
				_Device = new SsdpRootDevice(DescriptionLocation,CacheLifetime,System.Text.UTF8Encoding.UTF8.GetString(data,0,data.Length));
			}
		...
		public override string ToString() => Usn;
