	...
	internal static class TaskEx {
		...
		public static Task Delay( TimeSpan period ) {
			if( period.TotalMilliseconds > int.MaxValue ) {
				throw new ArgumentOutOfRangeException("period",string.Format("period cannot be more than {0} millseconds.",period.TotalMilliseconds));
			}
		...
		public static Task Delay( int millisecondsDelay ) {
			if( millisecondsDelay < -1 ) {
				throw new ArgumentOutOfRangeException("millisecondsDelay","millisecondsDelay must be -1 or greater.");
			}
