...
namespace Rssdp {
	...
	public sealed class SocketFactory : ISocketFactory {
		...
		public IUdpSocket CreateUdpSocket( int localPort ) {
			...
			Socket retVal = new Socket(_LocalIP.AddressFamily,System.Net.Sockets.SocketType.Dgram,System.Net.Sockets.ProtocolType.Udp);
			try {
		...
		public IUdpSocket CreateUdpMulticastSocket( int multicastTimeToLive,int localPort ) {
			...
			if( multicastTimeToLive <= 0 ) {
				throw new ArgumentException("multicastTimeToLive cannot be zero or less.","multicastTimeToLive");
			}
			...
			Socket retVal = new Socket(_LocalIP.AddressFamily,SocketType.Dgram,ProtocolType.Udp);
	...
	// See https://github.com/dotnet/corefx/pull/11509 for more details
		...
		/// </summary>
		public DeviceNetworkType DeviceNetworkType => _DeviceNetworkType;
