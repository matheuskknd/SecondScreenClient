...
namespace Rssdp {
	...
	public sealed class CustomHttpHeader {
		...
		private static void EnsureValidName( string name ) {
			...
			foreach( char c in name ){
				...
				byte b = (byte) c;
				if( c == ':' || b < 33 || b > 126 )
					throw new ArgumentException("Name contains illegal characters.",nameof(name));
			}
		}
	...
	public class CustomHttpHeadersCollection : IEnumerable<CustomHttpHeader> {
		...
		private readonly IDictionary<string,CustomHttpHeader> _Headers;
		...
		public CustomHttpHeadersCollection() {
			_Headers = new Dictionary<string,CustomHttpHeader>(StringComparer.OrdinalIgnoreCase);
		}
		...
		public bool Remove( string headerName ) {
			...
			if( string.IsNullOrEmpty(headerName) )
				throw new ArgumentException("headerName cannot be null or empty.",nameof(headerName));
		...
		public CustomHttpHeader this[string name] => _Headers[name];
