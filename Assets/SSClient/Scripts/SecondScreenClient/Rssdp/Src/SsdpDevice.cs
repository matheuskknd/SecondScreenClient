...
namespace Rssdp {
	...
	public abstract class SsdpDevice {
		...
		protected SsdpDevice() {
			...
			Icons = new List<SsdpDeviceIcon>();
			_Devices = new List<SsdpDevice>();
			Devices = new ReadOnlyEnumerable<SsdpDevice>(_Devices);
			...
			Services = new ReadOnlyEnumerable<SsdpService>(_Services);
		}
		...
		protected SsdpDevice( string deviceDescriptionXml ): this() {

			if( deviceDescriptionXml == null )
				throw new ArgumentNullException("deviceDescriptionXml");

			if( deviceDescriptionXml.Length == 0 )
				throw new ArgumentException("deviceDescriptionXml cannot be an empty string.","deviceDescriptionXml");

			using( System.IO.MemoryStream ms = new System.IO.MemoryStream(System.Text.UTF8Encoding.UTF8.GetBytes(deviceDescriptionXml)) ) {
		...
		public string DeviceTypeNamespace {
			...
			get => _DeviceTypeNamespace;
			set => _DeviceTypeNamespace = value;
		}
		...
		/// </remarks>
		public string FullDeviceType => string.Format("urn:{0}:device:{1}:{2}",
		...
		/// </summary>
		public IList<SsdpDeviceIcon> Icons { get; private set; }
		...
		/// <seealso cref="AddDevice"/>
		/// <seealso cref="RemoveDevice"/>
		public IEnumerable<SsdpDevice> Devices { get; private set; }
		...
		/// </summary>
		public SsdpDevicePropertiesCollection CustomProperties => _CustomProperties;
		...
		/// </remarks>
		public CustomHttpHeadersCollection CustomResponseHeaders => _CustomResponseHeaders;
		...
		/// <seealso cref="AddService"/>
		/// <seealso cref="RemoveService"/>
		public IEnumerable<SsdpService> Services { get; private set; }
		...
		protected virtual void OnDeviceAdded( SsdpEmbeddedDevice device ) {
			...
			EventHandler<DeviceEventArgs> handlers = DeviceAdded;
		...
		protected virtual void OnDeviceRemoved( SsdpEmbeddedDevice device ) {
			...
			EventHandler<DeviceEventArgs> handlers = DeviceRemoved;
		...
		protected virtual void OnServiceAdded( SsdpService service ) {
			...
			EventHandler<ServiceEventArgs> handlers = ServiceAdded;
		...
		protected virtual void OnServiceRemoved( SsdpService service ) {
			...
			EventHandler<ServiceEventArgs> handlers = ServiceRemoved;
		...
		private static void WriteCustomProperties( XmlWriter writer,SsdpDevice device ) {
			...
			foreach( SsdpDeviceProperty prop in device.CustomProperties )
		...
		private static void WriteIcons( XmlWriter writer,SsdpDevice device ) {
			...
			if( device.Icons.Any() ) {
				...
				foreach( SsdpDeviceIcon icon in device.Icons ) {
		...
		private void WriteChildDevices( XmlWriter writer,SsdpDevice parentDevice ) {
			...
			if( parentDevice.Devices.Any() ) {
				...
				foreach( SsdpDevice device in parentDevice.Devices )
		...
		private static void WriteServices( XmlWriter writer,SsdpDevice device ) {
			...
			if( device.Services.Any() ) {
				...
				foreach( SsdpService service in device.Services )
		...
		private static void LoadServices( XmlReader reader,SsdpDevice device ) {
			...
			while( !reader.EOF ) {
				...
				if( reader.LocalName == "service" ) {
					...
					SsdpService service = new SsdpService(reader.ReadOuterXml());
		...
		private static void LoadIcons( XmlReader reader,SsdpDevice device ) {
			...
			while( !reader.EOF ) {
				...
				if( reader.LocalName == "icon" ) {
					...
					SsdpDeviceIcon icon = new SsdpDeviceIcon();
		...
		private void LoadChildDevices( XmlReader reader,SsdpDevice device ) {
			...
			while( !reader.EOF ) {
				...
				if( reader.LocalName == "device" ) {
					...
					SsdpEmbeddedDevice childDevice = new SsdpEmbeddedDevice();
		...
		private static void AddCustomProperty( XmlReader reader,SsdpDevice device ) {
			...
			SsdpDeviceProperty newProp = new SsdpDeviceProperty() { Namespace = reader.Prefix,Name = reader.LocalName };
		...
		private bool ChildDeviceExists( SsdpDevice device ) =>
			(from d in _Devices where device.Uuid == d.Uuid select d).Any();

		private bool ChildServiceExists( SsdpService service ) =>
			(from s in _Services where service.Uuid == s.Uuid select s).Any();
