...
";
		...
		public Task<IEnumerable<DiscoveredSsdpDevice>> SearchAsync() =>
			SearchAsync(SsdpConstants.SsdpDiscoverAllSTHeader,DefaultSearchWaitTime);
		...
		public Task<IEnumerable<DiscoveredSsdpDevice>> SearchAsync( string searchTarget ) =>
			SearchAsync(searchTarget,DefaultSearchWaitTime);
		...
		public Task<IEnumerable<DiscoveredSsdpDevice>> SearchAsync( TimeSpan searchWaitTime ) =>
			SearchAsync(SsdpConstants.SsdpDiscoverAllSTHeader,searchWaitTime);
		...
		public async Task<IEnumerable<DiscoveredSsdpDevice>> SearchAsync( string searchTarget,TimeSpan searchWaitTime ) {
			...
			if( searchTarget == null )
				throw new ArgumentNullException("searchTarget");

			if( searchTarget.Length == 0 )
				throw new ArgumentException("searchTarget cannot be an empty string.","searchTarget");
			...
			if( _SearchResults != null )
				throw new InvalidOperationException("Search already in progress. Only one search at a time is allowed.");
			...
			lock( _SearchResultsSynchroniser ) {
				...
				foreach( DiscoveredSsdpDevice device in GetUnexpiredDevices().Where(NotificationTypeMatchesFilter) ) {
			...
			try {
				...
				Task expireTask = RemoveExpiredDevicesFromCacheAsync();
		...
		protected virtual void OnDeviceUnavailable( DiscoveredSsdpDevice device,bool expired ) {
			...
			EventHandler<DeviceUnavailableEventArgs> handlers = DeviceUnavailable;
		...
		private void AddOrUpdateDiscoveredDevice( DiscoveredSsdpDevice device ) {
			...
			lock( _Devices ) {
				...
				DiscoveredSsdpDevice existingDevice = FindExistingDeviceNotification(_Devices,device.NotificationType,device.Usn);
		...
		private void DeviceFound( DiscoveredSsdpDevice device,bool isNewDevice ) {
			...
			lock( _SearchResultsSynchroniser ) {
				...
				if( _SearchResults != null ) {
					...
					DiscoveredSsdpDevice existingDevice = FindExistingDeviceNotification(_SearchResults,device.NotificationType,device.Usn);
		...
		private void BroadcastDiscoverMessage( string serviceType,TimeSpan mxValue ) {
			...
			byte[] multicastMessage = BuildDiscoverMessage(serviceType,mxValue,multicastIpAddress);
		...
		private void ProcessSearchResponseMessage( HttpResponseMessage message ) {
			...
			Uri location = GetFirstHeaderUriValue("Location",message);
		...
		private void ProcessNotificationMessage( HttpRequestMessage message ) {
			...
			string notificationType = GetFirstHeaderStringValue("NTS",message);
		...
		private void ProcessByeByeNotification( HttpRequestMessage message ) {
			...
			string notficationType = GetFirstHeaderStringValue("NT",message);

			if( !string.IsNullOrEmpty(notficationType) ) {

				string usn = GetFirstHeaderStringValue("USN",message);

				if( !DeviceDied(usn,false) ) {

					DiscoveredSsdpDevice deadDevice = new DiscoveredSsdpDevice() {
		...
		private void ResetExpireCachedDevicesTimer() {
			...
			if( _ExpireCachedDevicesTimer == null )
				_ExpireCachedDevicesTimer = new Timer(ExpireCachedDevices,null,System.Threading.Timeout.Infinite,System.Threading.Timeout.Infinite);
		...
		private void RemoveExpiredDevicesFromCache() {
			...
			lock( _Devices ) {
				...
				foreach( DiscoveredSsdpDevice device in expiredDevices ) {
			...
			// problems.
			foreach( string expiredUsn in (from expiredDevice in expiredDevices select expiredDevice.Usn).Distinct() ) {
		...
		private bool DeviceDied( string deviceUsn,bool expired ) {
			...
			lock( _Devices ) {
				...
				foreach( DiscoveredSsdpDevice existingDevice in existingDevices ) {
			...
			if( existingDevices != null && existingDevices.Any() ) {
				...
				lock( _SearchResultsSynchroniser ) {
					...
					if( _SearchResults != null ) {
						...
						DiscoveredSsdpDevice[] resultsToRemove = (from result in _SearchResults where result.Usn == deviceUsn select result).ToArray();

						foreach( DiscoveredSsdpDevice result in resultsToRemove ) {
				...
				foreach( DiscoveredSsdpDevice removedDevice in existingDevices )
		...
		private static DiscoveredSsdpDevice FindExistingDeviceNotification( IEnumerable<DiscoveredSsdpDevice> devices,string notificationType,string usn ) =>
			(from d in devices where d.NotificationType == notificationType && d.Usn == usn select d).FirstOrDefault();
		...
		private static IEnumerable<DiscoveredSsdpDevice> FindExistingDeviceNotifications( IList<DiscoveredSsdpDevice> devices,string usn ) =>
			(from d in devices where d.Usn == usn select d).ToArray();
